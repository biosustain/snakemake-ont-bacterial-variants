...
Reads were filtered using `r paste(snakemake@config[["filtering"]])` to enrich for long high-quality reads:
```
`r paste(snakemake@config[["filtering"]])` `r paste(snakemake@config[[snakemake@config[["filtering"]]]][["params"]])` <RAWFASTQ>
```
<br>
...
Filtered reads were aligned to the supplied reference genome (`r unique(rawcov$contig)`) using the aligner `r paste(snakemake@config[["aligner"]])`
...
covplot <- ggplot(data = cov4plotting, aes(x = coverage.depth, y = value)) +
  ...
  scale_x_continuous(trans = "log10", guide = "axis_logticks")
covplot
```
...
Single nucleotide variants and indels were identified using three different tools, Clair3 and DeepVariant.
...
Clair3 is a deep learning-based variant caller (see [here](https://doi.org/10.1038/s43588-022-00387-x) and [here](https://github.com/HKU-BAL/Clair3)). Clair3 (v1.0.5) was run on the alignment files produced by `r paste(snakemake@config[["aligner"]])` using standard parameters:
```
...
if (nrow(clair) > 0) {
  ...
  formatStyle(
    "allele.frequency",
    backgroundColor = styleInterval(75, create_gradient(c(75), "0,255,0"))
  )
} else {
...
```
medaka_variant -m <BASECALLINGMODEL> -o <OUTPUTDIRECTORY>
-i <FASTQ> -r <REFERENCE>
```
...
NanoCaller is a neural network based variant caller (see [here](https://doi.org/10.1186/s13059-021-02472-2) and [here](https://github.com/WGLab/NanoCaller)). NanoCaller (v3.6.0) was run on the alignment files produced by `r paste(snakemake@config[["aligner"]])` using customized parameters (See config file for details)):
```
NanoCaller --bam <BAM> --ref <REFERENCE> --preset ont --output <OUTPUTDIRECTORY> ...

```

Nanocaller doesn't report/retrieve read depth and allele frequency for indels for now as reported [here](https://github.com/WGLab/NanoCaller/issues/46). They are left as NA.
Predicted SNVs and indels are listed below with information on:
...
# ```{r NanoCaller, echo=FALSE, results = 'asis'}
# # Functions for re-formatting variant information from NanoCaller VCF file
# # NanoCaller doesn't report depth or frequency for indels at the moment. This info will 
# # be missing for some calls: https://github.com/WGLab/NanoCaller/issues/46
# get_info_for_NanoCaller <- function(x, type) {
#   keys <- unlist(strsplit(x["format"], ":"))
#   values <- unlist(strsplit(x["sample"], ":"))
#   df <- data.frame(values, row.names = keys)
  
#   if (type == "read.depth") {
#     if ("DP" %in% rownames(df)) {
#       return(as.integer(df["DP", 1]))
#     } else {
#       return(NA)  # Return NA if DP is not available
#     }
#   }
  
#   if (type == "allele.frequency") {
#     if ("VF" %in% rownames(df)) {
#       return(round(100 * as.double(df["VF", 1]), 1))
#     } else {
#       return(NA)  # Return NA if AF is not available
#     }
#   }
# }
# # Import of NanoCaller VCF file
# NanoCaller <- read.table(as.character(snakemake@input[["NanoCaller"]]), sep = "\t", header = F,
#   col.names = c("contig", "position", "ID", "ref.allele", "alt.allele", "quality", "filter", "info", "format", "sample"),
#   colClasses = c("character", "integer", "character", "character", "character", "numeric", "character", "character", "character", "character"))
# if (nrow(NanoCaller) > 0) {
#   NanoCaller$read.depth <- apply(NanoCaller, 1, get_info_for_NanoCaller, type = "read.depth")
#   NanoCaller$allele.frequency <- apply(NanoCaller, 1, get_info_for_NanoCaller, type = "allele.frequency")
#   NanoCaller$ID <- NanoCaller$filter <- NanoCaller$info <- NanoCaller$format <- NanoCaller$sample <- NULL
#   NanoCaller$type <- apply(NanoCaller, 1, determine_snv_type)
#   NanoCaller <- NanoCaller[,c(1,2,8,3,4,5,6,7)]
#   # Create output table
#   DT::datatable(
#     NanoCaller,
#     colnames = c("ID", "contig", "position", "type", "reference allele", "variant allele", "quality", "read depth", "variant allele frequency (%)"),
...
#   )
# } else {
#   cat("No variants detected by NanoCaller")
# }
# ``` 
 -->
...
DeepVariant is a deep learning-based variant caller (see [here](https://doi.org/10.1038/nbt.4235) and [here](https://github.com/google/deepvariant)). DeepVariant (v1.0.5) was run on the alignment files produced by `r paste(snakemake@config[["aligner"]])` using non-standard parameters:
<!-- ```
run_DeepVariant.sh --bam_fn <BAM> --ref_fn <REFERENCE> --include_all_ctgs --haploid_sensitive
--platform ont --model_path <BASECALLINGMODEL> --output <OUTPUTDIRECTORY>
``` -->
...
```{r DeepVariant, echo=FALSE, results = 'asis'}
# Functions for re-formatting variant information from DeepVariant VCF file
get_info_for_DeepVariant <- function(x, type) {
  keys <- unlist(strsplit(x["format"], ":"))
  values <- unlist(strsplit(x["sample"], ":"))
  df <- data.frame(values, row.names = keys)
  if (type == "read.depth") {
  ...
  if (type == "allele.frequency") {
    return(round(100 * as.double(df["VAF",1]), 1))
  }
}
# Import of DeepVariant VCF file
DeepVariant <- read.table(as.character(snakemake@input[["DeepVariant"]]), sep = "\t", header = F,
  col.names = c("contig", "position", "ID", "ref.allele", "alt.allele", "quality", "filter", "info", "format", "sample"),
  colClasses = c("character", "integer", "character", "character", "character", "numeric", "character", "character", "character", "character"))
if (nrow(DeepVariant) > 0) {
  DeepVariant$read.depth <- apply(DeepVariant, 1, get_info_for_DeepVariant, type = "read.depth")
  DeepVariant$allele.frequency <- apply(DeepVariant, 1, get_info_for_DeepVariant, type = "allele.frequency")
  DeepVariant$ID <- DeepVariant$filter <- DeepVariant$info <- DeepVariant$format <- DeepVariant$sample <- NULL
  DeepVariant$type <- apply(DeepVariant, 1, determine_snv_type)
  DeepVariant <- DeepVariant[,c(1,2,8,3,4,5,6,7)]
  # Create output table
  DT::datatable(
    DeepVariant,
    colnames = c("ID", "contig", "position", "type", "reference allele", "variant allele", "quality", "read depth", "variant allele frequency (%)"),
  ...
  cat("No variants detected by DeepVariant")
}
```

### All tools {.active}

The following variants were identified by Clair3 and DeepVariant. Please note that the these three tools use different approaches to calculate the quality score of SNVs.

```{r commonsnvs, echo=FALSE, results = 'asis'}
# if ((nrow(clair) > 0 && nrow(medaka) > 0 && nrow(NanoCaller) > 0)) {
# if ((nrow(clair) > 0 && nrow(NanoCaller) > 0 && nrow(DeepVariant) > 0 )) {
# if ((nrow(clair) > 0 && nrow(NanoCaller) > 0)) {
if ((nrow(clair) > 0 && nrow(DeepVariant) > 0)) {
  ...
  # NanoCallerdf <- as.data.frame.matrix(NanoCaller) %>%
  #   select(-c("allele.frequency")) %>%
  #   rename("NanoCaller.quality" = "quality", "NanoCaller.read.depth" = "read.depth")
  DeepVariant <- as.data.frame.matrix(DeepVariant) %>%
    select(-c("allele.frequency")) %>%
    rename("DeepVariant.quality" = "quality", "DeepVariant.read.depth" = "read.depth")
  commonsnvs <- clairdf %>%
    # inner_join(medakadf, by = c("contig", "position", "type", "ref.allele", "alt.allele")) %>%
    # inner_join(NanoCallerdf, by = c("contig", "position", "type", "ref.allele", "alt.allele")) %>%
    inner_join(DeepVariant, by = c("contig", "position", "type", "ref.allele", "alt.allele"))
  DT::datatable(
    commonsnvs,
    ...
    # colnames = c("ID", "contig", "position", "type", "reference allele", "variant allele", "Clair3 quality score", "Clair3 read depth", "Medaka quality score", "Medaka read depth", "NanoCaller quality score", "NanoCaller read depth"),
    # colnames = c("ID", "contig", "position", "type", "reference allele", "variant allele", "Clair3 quality score", "Clair3 read depth", "NanoCaller quality score", "NanoCaller read depth", "DeepVariant quality score", "DeepVariant read depth"),
    colnames = c("ID", "contig", "position", "type", "reference allele", "variant allele", "Clair3 quality score", "Clair3 read depth", "DeepVariant quality score", "DeepVariant read depth"),
    filter = "top",
    options = 
      list(
        pageLength = 50
  ...
  # cat("No common variants detected by Clair3, Medaka and NanoCaller")
  # cat("No common variants detected by Clair3, NanoCaller and DeepVariant")
  cat("No common variants detected by Clair3 and DeepVariant")
}
```
...
cuteSV (v2.1.0) was run on `r paste(snakemake@config[["aligner"]])`-aligned reads using standard settings suggested for ONT data:
...
Sniffles2 is a structural variant caller designed for long-read sequencing data aligned with the long-read mapper `r paste(snakemake@config[["aligner"]])` (see [here](https://doi.org/10.1038/s41592-018-0001-7) for the original Sniffles,
...
[Breseq](https://github.com/barricklab/breseq) is a complete pipeline for filtering, mapping reads; then calling and annotating mutations for bacterial re-sequencing studies. It is developed for using high quality short reads. Recently, long read (pseudo-)support is added with a workaround to chop reads down to 200bp (or selected configuration). 
```{r breseq, echo=FALSE, results = 'asis'}
# Functions for re-formatting variant information from Breseq VCF file.
# Breseq reports the called mutation frequency at "1.)" for consensus if the actual frequency is over the cut-off selected (0.80 by default). Remaining mutation calls are reported as marginal.
get_info_for_breseq <- function(x, type) {
  # Split the info field by semicolons into individual key-value pairs
  info_pairs <- strsplit(x["info"], ";")[[1]]
  
  # Create a named list by splitting each key-value pair at the '=' character
  info_list <- setNames(sapply(info_pairs, function(pair) strsplit(pair, "=")[[1]][2]),
                        sapply(info_pairs, function(pair) strsplit(pair, "=")[[1]][1]))
  
  # Return the requested value as numeric if it exists, otherwise NA
  if (type %in% names(info_list)) {
    return(as.numeric(info_list[type]))
  } else {
    return(NA)  # Return NA if the requested type is not available
  }
}

# Import of breseq VCF file
# Read all columns as character to prevent initial type mismatch errors
breseq <- read.table(as.character(snakemake@input[["breseq"]]), sep = "\t", header = F,
  col.names = c("contig", "position", "ID", "ref.allele", "alt.allele", "quality", "filter", "info"),
  colClasses = "character")
# Convert columns to their intended types
breseq$contig <- as.character(breseq$contig)
breseq$position <- as.integer(breseq$position)
breseq$ID <- as.character(breseq$ID)
breseq$ref.allele <- as.character(breseq$ref.allele)
breseq$alt.allele <- as.character(breseq$alt.allele)
breseq$filter <- as.character(breseq$filter)
breseq$info <- as.character(breseq$info)
# Convert quality column, treating "." as NA since breseq leaves . in case of longer deletions, insertions for quality.
breseq$quality <- as.numeric(ifelse(breseq$quality == ".", NA, breseq$quality))
if (nrow(breseq) > 0) {
  breseq$read.depth <- apply(breseq, 1, get_info_for_breseq, type = "DP")
  breseq$allele.frequency <- apply(breseq, 1, get_info_for_breseq, type = "AF")
  breseq$ID <- breseq$filter <- breseq$info <- NULL
  breseq$type <- apply(breseq, 1, determine_snv_type)
  breseq <- breseq[,c(1,2,8,3,4,5,6,7)]
  # Create output table
  DT::datatable(
    breseq,
    colnames = c("ID", "contig", "position", "type", "reference allele", "variant allele", "quality", "read depth", "variant allele frequency (%)"),
  ...
  cat("No variants detected by breseq")
}
```
...
<!-- * Nanocaller: `r qvalues$NanoCaller` -->
* DeepVariant: `r qvalues$DeepVariant`
...
<!-- #### NanoCaller
# ```{r shared-NanoCaller, echo=FALSE, warning=FALSE, results = 'asis'}
# if (snakemake@config[["remove_common_variants"]]) {
#   sharedByNanoCaller <- read.table(as.character(sharedVariantList[[2]]), sep = "\t", header = F,,
#     col.names = c("contig", "position", "ID", "ref.allele", "alt.allele", "quality", "filter", "info", "format", "sample"),
#     colClasses = c("character", "integer", "character", "character", "character", "numeric", "character", "character", "character", "character"))
#   if (nrow(sharedByNanoCaller) > 0) {
#     sharedByNanoCaller$type <- apply(sharedByNanoCaller, 1, determine_snv_type)
#     sharedByNanoCaller <- sharedByNanoCaller[,c(1,2,11,4,5)]
#     DT::datatable(
#         sharedByNanoCaller,
#         colnames = c("contig", "position", "type", "reference allele", "variant allele"),
#         filter = "top",
...
#       )
#   } else {
#     cat("No common variants detected for all samples by NanoCaller.\n")
#   }
# } else{
#   cat("No filtering by shared variants enabled!")
# }
...
#### DeepVariant
```{r shared-DeepVariant, echo=FALSE, warning=FALSE, results = 'asis'}
if (snakemake@config[["remove_common_variants"]]) {
  sharedByDeepVariant <- read.table(as.character(sharedVariantList[[1]]), sep = "\t", header = F,
    col.names = c("contig", "position", "ID", "ref.allele", "alt.allele", "quality", "filter", "info", "format", "sample"),
    colClasses = c("character", "integer", "character", "character", "character", "numeric", "character", "character", "character", "character"))
  if (nrow(sharedByDeepVariant) > 0) {
    sharedByDeepVariant$type <- apply(sharedByDeepVariant, 1, determine_snv_type)
    sharedByDeepVariant <- sharedByDeepVariant[,c(1,2,11,4,5)]
    DT::datatable(
        sharedByDeepVariant,
        colnames = c("contig", "position", "type", "reference allele", "variant allele"),
        filter = "top",
    ...
    cat("No common variants detected for all samples by DeepVariant.\n")
  }
} else{
  cat("No filtering by shared variants enabled!")
}
